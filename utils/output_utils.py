...
"""def draw_img(ids_p, class_p, box_p, mask_p, img_origin, cfg, img_name=None, fps=None, low_d=250, high_d=2880):
    _img_origin = (img_origin.astype(np.float) - (low_d + 1))
    _img_origin[_img_origin < 0] = 0
    img_origin = _img_origin / (high_d - low_d)
    img_origin *= 255.0
    ...
    if not cfg.hide_mask:
        ...
        masks_semantic = masks_semantic.astype('int').sum(axis=0) % (cfg.num_classes)
        color_masks = COLORS[masks_semantic].astype('uint8')
        img_fused = cv2.addWeighted(color_masks, 0.4, img_origin, 1.0, gamma=0)
        ...
        if cfg.cutout:
            for i in range(num_detected):
                ...
                cv2.imwrite(f'results/images/{img_name}_{i}.png', img_matting)
...
def draw_img(ids_p, class_p, box_p, mask_p, img_origin, cfg, img_name=None, fps=None):
    ...
    if not cfg.hide_mask:
       ...
       # cv2.imwrite('img_fused.png',img_fused)
       # cv2.imwrite('img_origin.png',img_origin)

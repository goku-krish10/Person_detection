...
def train_collate(batch):
    ...
    for sample in valid_batch:
        imgs.append(torch.tensor(sample[0].copy(), dtype=torch.float32))
        targets.append(torch.tensor(sample[1].copy(), dtype=torch.float32))
        masks.append(torch.tensor(sample[2].copy(), dtype=torch.float32))
...
class COCODetection(data.Dataset):
    def __init__(self, cfg, mode='train'):
        ...
        if mode in ('train', 'val'):
            ...
            self.image_path = glob.glob(cfg.image + '/*.png')
    ...
    def __getitem__(self, index):
        if self.mode == 'detect':
            ...
            img_origin = cv2.imread(img_name, cv2.IMREAD_ANYDEPTH)
            ...
            #print('targer after change :',target)
            ...
            #print('CONTINUOUS ID : ',self.continuous_id)
            for aa in target:
                bbox = aa['bbox']

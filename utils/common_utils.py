...
class APDataObject:
    ...
    def get_ap(self) -> float:
        ...
        y_range = [0] * 100  # idx 0 is recall == 0.0 and idx 100 is recall == 1.00
        x_range = np.array([(x+0.5) / 100 for x in range(101)])
        ...
        return sum(y_range) / len(y_range), precisions, recalls
...
def calc_map(ap_data, iou_thres, num_classes, step, pr_graph=False):
    print('\nCalculating mAP...')
    ...
    for _class in range(num_classes):
        filename_csv = "./output/yolact_iee101_" + dt_string + '.csv'
        csv_file_pr_file = open(filename_csv, 'w', newline='', encoding='utf-8')
        csv_file_pr = csv.writer(csv_file_pr_file, delimiter=";")
        for iou_idx in range(len(iou_thres)):
            for iou_type in ('box', 'mask'):
                ap_obj = ap_data[iou_type][iou_idx][_class]

                if not ap_obj.is_empty():
                    app, ps, rs = ap_obj.get_ap()
                    if (pr_graph):
                        csv_file_pr.writerow(["iou:", iou_thres[iou_idx], "type:", iou_type])
                        _ps = ["ps:"]
                        _rs = ["rs:"]
                        for p in ps:
                            _ps.append(p) 
                        for r in rs:
                            _rs.append(r)
                        csv_file_pr.writerow(_ps)
                        csv_file_pr.writerow(_rs)
                        filename = "./output/yolact_iee101_" + \
                                   "{:.2f}".format(iou_thres[iou_idx]) + \
                                   "_" + iou_type + "_" + dt_string + '.png'
                        plt.plot(rs, ps)
                        plt.ylim(0, 1)
                        plt.xlabel('Recall')
                        ...
                        plt.clf()
                    aps[iou_idx][iou_type].append(app)
        csv_file_pr_file.close()

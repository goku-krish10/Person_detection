...
parser.add_argument('--weight', default='trt_files/best_iee101_dataset.trt', type=str)
...
if cfg.image is not None:
    ...
    for i, (img, img_origin, img_name) in enumerate(data_loader):
        ...
        with timer.counter('save_img', trt_mode=True):
            ...
            cv2.imwrite(f'results/images_tensorrt/{img_name}', img_numpy)
    ...
    print('\nFinished, saved in: results/images_tensorrt.')
...
# detect videos
    ...
    video_writer = cv2.VideoWriter(f'results/videos_tensorrt/{name}', cv2.VideoWriter_fourcc(*"mp4v"), target_fps,
    ...
    if not cfg.real_time:
        print(f'\n\nFinished, saved in: results/videos_tensorrt/{name}')

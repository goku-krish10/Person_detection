...
if cfg.image is not None:
    ...
    for i, (img, img_origin, img_name) in enumerate(data_loader):
        if i == 1:
            timer.start()
           # print('Orig img',img_origin.shape)
           # print('Norm img',img.shape)
           # cv2.imwrite('Real_img.png',img_origin)
           # img_array = img.numpy()
           # print('Img array',img_array.shape)
           # img_new = np.random.randint(0, 100, size=(3, 512, 512))
           # for x in range(0,3):
           #     for y in range(0,512):
           #         for z in range(0,512):
           #             img_new[x][y][z] = img_array[0][x][y][z]

           # img_fin = np.transpose(img_new, (1,2,0))
           # cv2.imwrite('New_img.png',img_fin)
       ...
       # print('Masks',masks_p)
       # masks_array = masks_p.cpu().numpy()
       # print('Masks shape',masks_array.shape)
       # for x in range(0,512):
       #     for y in range(0,512):
       #         if(masks_array[0][x][y] != 0):
       #                 print('X and Y :',x,y)

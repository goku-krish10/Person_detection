...
class ResNet(nn.Module):
    ...
    def init_backbone(self, path, random=False):
        """ Initializes the backbone weights for training. """
        state_dict = torch.load(path)
        self.load_state_dict(state_dict, strict=True)
        if (random):
            for _, layer in enumerate(state_dict):
                shape = len(state_dict[layer].shape)
                if shape == 1:
                    state_dict[layer] = torch.nn.init.uniform_(state_dict[layer])
                else:
                    state_dict[layer] = torch.nn.init.kaiming_normal_(state_dict[layer])

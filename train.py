...
parser.add_argument('--cfg', default='iee101_dataset', help='The configuration name to use.')
parser.add_argument('--train_bs', type=int, default=8, help='total training batch size')
parser.add_argument('--img_size', default=512, type=int, help='The image size for training.')
parser.add_argument('--resume', default=None, type=str, help='The path of the weight file to resume training with.')
parser.add_argument('--val_interval', default=126613//10, type=int,
...
parser.add_argument('--num_im', default=126613, type=int, help='Number of images for training.')
parser.add_argument('--finetuning', default=False, action='store_true', help='Whether to switch to ITOP finetuning dataset.')

rnd_ind = 7
# for numpy randomness
import numpy as np
np.random.seed(rnd_ind)
# # for randomness in image augmentation
import random
random.seed(rnd_ind)
# # every PyTorch thing can be fixed with these two lines
torch.manual_seed(rnd_ind)
torch.cuda.manual_seed_all(rnd_ind)
...
if args.resume:
    assert re.findall(r'iee.+_[a-z]+', args.resume)[0] == cfg_name, 'Resume weight is not compatible with current cfg.'	
...
if cfg.__class__.__name__ == 'iee101_dataset':
    optimizer = optim.SGD(net.parameters(), lr=cfg.lr, momentum=0.9, weight_decay=1e-3)
    ...
    while training:
        ...
        for images, targets, masks in data_loader:
            ...
            with timer.counter('for+loss'):
                loss_c, loss_b, loss_m, loss_s, loss_n = net(images, targets, masks)

                if cfg.cuda:
                    # use .all_reduce() to get the summed loss from all GPUs
                    all_loss = torch.stack([loss_c, loss_b, loss_m, loss_s, loss_n], dim=0)
                    dist.all_reduce(all_loss)

            with timer.counter('backward'):
                loss_total = loss_c + loss_b + loss_m + loss_s + loss_n
            ...
            if step % 100 == 0 and step != start_step:
                if (not cfg.cuda) or main_gpu:
                    ...
                    l_n = all_loss[4].item() / num_gpu if main_gpu else loss_n.item()
                    ...
                    writer.add_scalar('loss/surf_n', l_n, global_step=step)
                    writer.add_scalar('loss/total', loss_total, global_step=step)

                    print(f'step: {step} | lr: {cur_lr:.2e} | l_class: {l_c:.3f} | l_box: {l_b:.3f} | '
                          f'l_mask: {l_m:.3f} | l_semantic: {l_s:.3f} | l_surf_n: {l_n:.3f} | t_t: {t_t:.3f} | t_d: {t_d:.3f} | '

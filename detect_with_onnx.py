...
parser = argparse.ArgumentParser(description='YOLACT Detection with ONNX.')
parser.add_argument('--weight', default='onnx_files/iee101_dataset.onnx', type=str)
parser.add_argument('--image', default='data/dfki_dataset/depth/', type=str, help='The folder of images for detecting.')
...
parser.add_argument('--visual_thre', default=0.9, type=float,
                    help='Detections with a score under this threshold will be removed.')

args = parser.parse_args()
args.cfg = re.findall(r'iee.+_[a-z]+', args.weight)[0]
...
if cfg.image is not None:
    ...
    for i, (img, img_origin, img_name) in enumerate(data_loader):
        ...
        with timer.counter('forward'):
            class_p, box_p, coef_p, proto_p = sess.run(None, {input_name: img})
        ...
        with timer.counter('save_img'):
            ...
            cv2.imwrite(f'results/images_onnx/{img_name}', img_numpy)
    ...
    print('\nFinished, saved in: results/images_onnx.')
...
# detect videos
    ...
    video_writer = cv2.VideoWriter(f'results/videos_onnx/{name}', cv2.VideoWriter_fourcc(*"mp4v"), target_fps,
    ...
    for i in range(num_frames):
        ...
        with timer.counter('forward'):
            class_p, box_p, coef_p, proto_p = sess.run(None, {input_name: frame_trans})
    ...
    if not cfg.real_time:
        print(f'\n\nFinished, saved in: results/videos_onnx/{name}')
